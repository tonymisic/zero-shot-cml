...
from utils import GeneralizedZeroShot, ContrastiveLoss, create_mask
...
while epoch <= wandb.config['epochs']:
    ...
    for video, audio, temporal_labels, spatial_labels, class_names, back_start, back_end in train_loader:
        ...
        audio_attention = audio_attention_model(local_audio.permute([1,0,2])).permute([1,0,2])
        video_attention = video_attention_model(video.permute([1,0,2])).permute([1,0,2])
        ...
        for i in range(batch_size):
            ...
            query_video_attention = video_attention_model(query_video.unsqueeze(0)).permute([1,0,2]).squeeze(0)
            query_audio_attention = audio_attention_model(query_audio.unsqueeze(0)).permute([1,0,2]).squeeze(0)
            V2A_accuracies[i] = max_similarity(query_video_attention[0].squeeze(0), audio_attention[i], back_start[i])
            A2V_accuracies[i] = max_similarity(query_audio_attention[0].squeeze(0), video_attention[i], back_start[i])
        A2V_ACCURACY, V2A_ACCURACY = float(torch.sum(A2V_accuracies)) / batch_size, float(torch.sum(V2A_accuracies)) / batch_size
        ######### ------------------------------ LOSS AND RECORD ---------------------------------------- ###########
        labels = create_mask(back_start).to(device)
        LOSS = criterion(audio_attention, video_attention, labels, device)
        LOSS.backward()
    ...
    for video, audio, temporal_labels, spatial_labels, class_names, back_start, back_end in val_loader:
        if torch.sum(temporal_labels[0]) < 10:
            ...
            running_temporalA2V += A2V_ACCURACY
    print("Samples: " + str(batch))
    wandb.log({"Eval A2V": running_temporalA2V / batch})
    wandb.log({"Eval V2A": running_temporalV2A / batch})
    epoch += 1
